        ...
        <div className="flex items-center gap-1 p-1 bg-white/[0.03] border border-white/8 rounded-2xl">
          {(["all-time","monthly","weekly"] as Period[]).map(p => (
            <button
              key={p}
              onClick={() => setPeriod(p)}
              className={`relative px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wider transition-all ${
                period === p ? "text-white" : "text-white/35 hover:text-white"
              }`}
            >
              {period === p && (
          ...
          return (
            <TiltCard key={user.rank} intensity={6} glare className={`flex flex-col items-center`}>
              ...
              >
                ...
                {/* Avatar */}
                <div className={`relative w-16 h-16 rounded-2xl bg-gradient-to-br ${grad} p-0.5 mb-3 shadow-lg`}>
                  <div className="w-full h-full rounded-[calc(1rem-2px)] bg-[#0a0a0a] flex items-center justify-center text-xl font-black">
                    {user.name[0]}
                  </div>
              ...
              <motion.div
                ...
                className={`grid grid-cols-[2rem_1fr_5rem_5rem_5rem_5rem] gap-4 items-center px-5 py-4 rounded-2xl border transition-all ${
                  isMe
                    ...
                    : "border-white/5 bg-white/[0.01] hover:bg-white/[0.03] hover:border-white/12"
                }`}
              >
                {/* Rank */}
                ...
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-600/40 to-violet-600/30 flex items-center justify-center font-black text-sm shrink-0">
                    {user.name[0]}
                  </div>
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <p className={`text-sm font-black truncate ${isMe ? "text-indigo-300" : "text-white/80"}`}>
                        {user.name}{isMe && " (You)"}
                      </p>
