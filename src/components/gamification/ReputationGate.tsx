    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          <button
            onClick={onClose}
            className="absolute top-5 right-5 p-1.5 rounded-full text-white/30 hover:text-white/70 hover:bg-white/5 transition-all"
            aria-label="Close"
          >
            <X size={16} />
          </button>
          ...
          {/* Heading */}
          <h2 className="text-2xl font-black tracking-tighter text-center text-white mb-1">
            Reputation Required
          </h2>
          <p className="text-center text-white/40 text-sm mb-6">
            <span className="text-white/70 font-bold">{feature}</span> is locked behind reputation.
          </p>
          ...
          <div className="space-y-2 mb-6">
            <div className="flex justify-between text-xs font-black uppercase tracking-tighter">
              ...
              <span className="text-white/40">Required: {required}</span>
            </div>
            ...
            <p className="text-right text-[10px] text-white/30 font-bold uppercase tracking-tighter">
          ...
          <div className="space-y-3">
            <p className="text-xs font-black uppercase tracking-tighter text-white/30">How to earn reputation</p>
            <div className="space-y-2">
              {HOW_TO_EARN.map(({ action, points }) => (
                <div
                  key={action}
                  className="flex items-center justify-between px-4 py-3 rounded-2xl bg-white/[0.03] border border-white/5"
                >
                  <span className="text-sm text-white/70 font-bold">{action}</span>
          ...
          <button
            onClick={onClose}
            className="mt-8 w-full py-3 rounded-2xl bg-white/5 border border-white/10 text-white/60 font-black text-sm uppercase tracking-tighter hover:bg-white/10 hover:text-white transition-all"
          >
            Got it
          </button>
        </motion.div>
