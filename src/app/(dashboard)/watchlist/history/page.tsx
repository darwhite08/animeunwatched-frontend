...
function EpisodeRow({ entry, index }: { entry: EpisodeEntry; index: number }) {
  return (
    ...
    <Link
      ...
      className="flex items-center gap-3 sm:gap-4 px-3 sm:px-4 py-3 min-h-11 rounded-2xl bg-surface border border-border hover:border-white/30 active:scale-[0.99] transition-all group"
    >
      {/* Cover thumbnail */}
  ...
  return (
    <div className={`max-w-5xl mx-auto ${ui.screenX} py-8 sm:py-12 pb-32 space-y-8 sm:space-y-10`}>
      ...
      <div className="flex items-center gap-3 pt-4 border-t border-border">
        <Link
          href="/watchlist"
          className={`inline-flex items-center gap-2 ${ui.touch} text-[11px] font-black uppercase tracking-widest text-subtle hover:text-foreground active:scale-95 transition-all`}
        >
          ← Back to Watchlist
        </Link>
      </div>
