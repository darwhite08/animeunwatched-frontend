    ...
    <div className="p-8 rounded-[2.5rem] border border-border bg-surface relative overflow-hidden group">
      ...
      <div className="flex items-center justify-between mb-6 relative z-10">
        <div className="flex items-center gap-2">
          ...
          <h4 className="text-xs font-black uppercase tracking-[0.28em] text-subtle">Airing Today</h4>
      ...
      {isLoading ? (
        <div className="space-y-3">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="flex items-center gap-3 animate-pulse">
              <div className="h-10 w-7 rounded-lg bg-surface shrink-0" />
              <div className="flex-1 space-y-1.5">
                <div className="h-2.5 w-2/3 bg-surface rounded-full" />
                <div className="h-2 w-1/3 bg-surface rounded-full" />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="text-center py-6 space-y-2">
          <CalendarCheck size={20} className="mx-auto text-subtle" />
          <p className="text-[10px] font-black uppercase tracking-widest text-subtle">No data for today</p>
        ...
        <div className="space-y-3 relative z-10">
            ...
            >
              <Link href={`/anime/${anime.malId}`}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[11px] font-bold text-muted group-hover/item:text-foreground transition-colors leading-tight truncate">
                    {anime.title}
                  </p>
                  <p className="text-[9px] text-subtle font-bold mt-0.5">
                    {anime.studios?.[0] ?? "Unknown Studio"} · {anime.type ?? "TV"}
                  </p>
                </div>
                {anime.score && (
                  <span className="text-[9px] font-black text-accent-bright/70 shrink-0">★ {anime.score.toFixed(1)}</span>
                )}
              </Link>
            </motion.div>
          ))}
          ...
          {tomorrowAnime.length > 0 && (
            <div className="pt-3 border-t border-border space-y-1.5">
              <p className="text-[8px] font-black uppercase tracking-[0.3em] text-subtle">Tomorrow ({DAYS[(today + 1) % 7]})</p>
              {tomorrowAnime.map((anime: AnimeDTO) => (
                <Link key={anime.malId} href={`/anime/${anime.malId}`}
                  className="flex items-center gap-2 text-[10px] text-subtle hover:text-muted transition-colors truncate font-medium">
