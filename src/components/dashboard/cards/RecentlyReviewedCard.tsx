  ...
  return (
    <div className="flex items-center gap-0.5">
      {Array.from({ length: full }).map((_, i) => (
        <Star key={`f-${i}`} size={9} className="text-accent-bright" fill="currentColor" />
      ))}
      {half === 1 && (
        <Star size={9} className="text-accent-bright/50" fill="currentColor" />
      )}
      {Array.from({ length: empty }).map((_, i) => (
        <Star key={`e-${i}`} size={9} className="text-subtle" fill="currentColor" />
      ))}
      <span className="ml-1.5 text-[9px] font-black text-muted">{score}/10</span>
    </div>
  )
}
    ...
    <div className="p-8 rounded-[2.5rem] border border-border bg-surface relative overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-7 relative z-10">
        <div className="flex items-center gap-2">
          ...
          <h4 className="text-xs font-black uppercase tracking-[0.28em] text-subtle">Recent Reviews</h4>
        </div>
        <button
          ...
          {reviews.map((review, i) => (
            ...
            >
              ...
              <div className="flex items-start justify-between gap-3 mb-2">
                ...
                <span className="text-[9px] text-subtle font-mono shrink-0 pt-0.5">{review.date}</span>
              </div>
              ...
              {/* Excerpt */}
              <p className="mt-2 text-[11px] text-muted leading-relaxed line-clamp-2 italic">
                "{review.excerpt}"
              </p>
            </motion.li>
          ))}
