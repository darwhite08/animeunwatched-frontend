  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-50 border-t border-border bg-background/95 backdrop-blur-xl">
      <div className="mx-auto flex max-w-5xl items-center gap-3 px-4 py-3 sm:gap-4">
        ...
        <Link
          href="/register"
          className="ml-auto inline-flex shrink-0 items-center gap-1.5 rounded-full px-5 py-2.5 text-[11px] font-black uppercase tracking-widest text-black transition-transform hover:scale-105"
          style={{ background: "linear-gradient(135deg, var(--app-accent-bright), var(--app-accent))" }}
        >
          Sign up free <ArrowRight size={13} />
        </Link>
        <Link href="/login" className="hidden shrink-0 rounded-full border border-border px-5 py-2.5 text-[11px] font-black uppercase tracking-widest text-muted hover:text-foreground sm:inline">
          Log in
        </Link>
        <button onClick={() => setDismissed(true)} aria-label="Dismiss" className="shrink-0 rounded-lg p-1.5 text-subtle hover:text-foreground">
          <X size={16} />
        </button>
      </div>
    </div>
