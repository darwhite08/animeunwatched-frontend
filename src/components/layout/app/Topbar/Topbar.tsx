  ...
  return (
    <>
      <header className="sticky top-0 z-40 flex h-[calc(4rem+env(safe-area-inset-top))] items-center gap-2 border-b border-white/[0.06] bg-gradient-to-b from-background/95 to-background/75 px-3 pt-[env(safe-area-inset-top)] backdrop-blur-xl sm:gap-3 sm:px-4">
        ...
        {/* Right corner only: Search + Create (notifications & profile live in the
            sidebar now). Search is an icon button; Create is a "+". */}
        <div className="ml-auto flex shrink-0 items-center gap-2">
          <button
            onClick={() => setSearchOpen(true)}
            aria-label="Search"
            title="Search (⌘K)"
            className="group flex h-11 w-11 items-center justify-center rounded-full border border-white/[0.07] bg-white/[0.03] text-muted shadow-[inset_0_1px_0_rgba(255,255,255,0.03)] transition-all hover:border-white/15 hover:bg-white/[0.05] hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/40"
          >
            <Search size={18} className="text-subtle transition-colors group-hover:text-foreground" />
          </button>
          <CreateButton />
        </div>
      </header>
