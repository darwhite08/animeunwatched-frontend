        ...
        <div className="border border-white/10 bg-white/[0.03] backdrop-blur-xl rounded-3xl p-8">
          {sent ? (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center space-y-5">
              ...
              <div>
                <h1 className="text-2xl font-black uppercase italic tracking-tight text-white">Check your inbox</h1>
                <p className="text-sm text-white/40 mt-2 leading-relaxed">
                  If <span className="text-white">{email}</span> is registered with us, we&apos;ve sent a password reset link. It expires in 1 hour.
                </p>
              </div>
              <p className="text-[10px] text-white/25 leading-relaxed">
                Didn&apos;t receive it? Check your spam folder. Still nothing? The email might not be registered.
              </p>
              <Link href="/login" className="inline-block text-xs font-black uppercase tracking-widest text-amber-400 hover:text-amber-300 transition-colors">
                Back to login →
              </Link>
            </motion.div>
          ) : (
            <>
              <div className="text-center mb-6">
                <h1 className="text-2xl font-black uppercase italic tracking-tight text-white">Reset Password</h1>
                <p className="text-sm text-white/40 mt-2">Enter your email and we&apos;ll send you a reset link.</p>
              </div>

              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-[9px] font-black uppercase tracking-[0.3em] text-white/35 mb-1.5">Email</label>
