...
/* ── Round action button ──────────────────────────────────────────────────── */
function Btn({
  onClick, bg = "rgba(255,255,255,0.12)", color = "#fff",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
      ...
      {label && <span style={{ fontSize: 11, color: "rgba(255,255,255,0.55)", fontWeight: 500 }}>{label}</span>}
    </div>
  )
}
  ...
  return src
    ? <Image src={src} alt={name} width={size} height={size}
        style={{ width: size, height: size, borderRadius: "50%", objectFit: "cover",
          boxShadow: "0 0 0 4px rgba(255,255,255,0.15), 0 8px 32px rgba(0,0,0,0.5)" }} />
    : <div style={{
        width: size, height: size, borderRadius: "50%",
  ...
  return (
    <motion.div initial={{ opacity: 0, y: -20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -20, scale: 0.95 }}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 16 }}>
        ...
        <div>
          <div style={{ fontSize: 15, fontWeight: 700, color: "#fff", marginBottom: 3 }}>{callerName}</div>
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      {callType === "audio" && (
        <div style={{ position: "relative", zIndex: 2, display: "flex", flexDirection: "column", alignItems: "center", gap: 20 }}>
          ...
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: 22, fontWeight: 700, color: "#fff", marginBottom: 6 }}>{peerName}</div>
            <div style={{ fontSize: 14, color: isCalling ? "oklch(0.78 0.16 145)" : "rgba(255,255,255,0.55)" }}>
              {statusLabel}
            </div>
          </div>
      ...
      {callType === "video" && (
        <div style={{ position: "relative", zIndex: 2, textAlign: "center" }}>
          <div style={{ fontSize: 20, fontWeight: 700, color: "#fff", textShadow: "0 2px 8px rgba(0,0,0,0.7)" }}>{peerName}</div>
          <div style={{ fontSize: 13, color: "rgba(255,255,255,0.65)", marginTop: 4 }}>{statusLabel}</div>
        </div>
      )}
      ...
      {callType === "video" && (
        <motion.div drag dragMomentum={false}
          style={{ position: "absolute", bottom: 140, right: 20, zIndex: 5, borderRadius: 14, overflow: "hidden", width: 110, height: 160, boxShadow: "0 4px 20px rgba(0,0,0,0.6)", border: "2px solid rgba(255,255,255,0.2)", cursor: "grab" }}>
      ...
      <div style={{ position: "relative", zIndex: 2, display: "flex", alignItems: "flex-end", gap: 20, justifyContent: "center", width: "100%" }}>
        {callType === "video" && (
          <Btn onClick={onCamera} label={isCamOff ? "Camera" : "Camera"}
            bg={isCamOff ? "rgba(255,255,255,0.15)" : "rgba(255,255,255,0.15)"}
            icon={isCamOff
