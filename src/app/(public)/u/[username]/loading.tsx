    ...
    <div className="min-h-screen bg-background animate-pulse">
      ...
      <div className="h-64 bg-surface border-b border-border">
        <div className="max-w-6xl mx-auto px-6 pt-24 flex items-end gap-6">
          <div className="h-24 w-24 rounded-3xl bg-surface mb-[-48px]" />
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="aspect-[2/3] rounded-2xl bg-surface" />
            ))}
          </div>
        </div>
        <div className="space-y-6">
          <div className="h-48 rounded-[2rem] bg-surface border border-border" />
          <div className="h-32 rounded-[2rem] bg-surface border border-border" />
        </div>
      </div>
    </div>
  )
