...
export const WatchStatsCard = () => {
  ...
  const stats = [
    { label: "Total Hours",  value: realStats.total > 0 ? String(realStats.totalHrs.toLocaleString()) : "—",    icon: Clock,        color: "text-accent-bright",  bg: "bg-accent/10"  },
      ...
      {stats.map((stat, i) => (
        <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
          ...
          <div className="space-y-1">
            ...
            <p className="text-[10px] font-black text-subtle uppercase tracking-[0.2em]">{stat.label}</p>
          </div>
        </motion.div>
      ))}
    </div>
