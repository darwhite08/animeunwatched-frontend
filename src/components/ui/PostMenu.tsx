      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 top-full mt-1 z-50 min-w-[180px] rounded-xl border border-border bg-surface/95 backdrop-blur-md shadow-2xl overflow-hidden"
          >
  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-2.5 px-3.5 py-2.5 text-[11px] font-bold uppercase tracking-widest transition-colors ${
        danger ? "text-red-300/80 hover:text-red-200 hover:bg-red-500/10" : "text-muted hover:text-foreground hover:bg-surface"
      }`}
    >
      <Icon size={12} />
      {label}
