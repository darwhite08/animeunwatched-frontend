    ...
    <div className="space-y-12">
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          {chars.length > 12 && (
            <button onClick={() => setShowAllChars(v => !v)} className="-mr-2 flex min-h-11 items-center px-2 text-[10px] font-black uppercase tracking-widest text-accent-bright hover:text-accent active:scale-95 transition-transform">
        ...
        {characters.isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3">
            {charsShown.map((c, i) => {
              ...
              return (
                <div key={`${c.character.mal_id}-${i}`} className="flex items-center justify-between gap-2 rounded-2xl border border-border bg-white/[0.02] p-2.5 active:scale-[0.99] transition-transform">
                  {/* Character */}
      ...
      {wantStaff && (staff.isLoading || staffList.length > 0) && (
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            {staffList.length > 8 && (
              <button onClick={() => setShowAllStaff(v => !v)} className="-mr-2 flex min-h-11 items-center px-2 text-[10px] font-black uppercase tracking-widest text-accent-bright hover:text-accent active:scale-95 transition-transform">
          ...
          {staff.isLoading ? (
            ...
            <div className="grid grid-cols-2 gap-2.5 sm:gap-3">
              {staffShown.map((s, i) => (
                <div key={`${s.person.mal_id}-${i}`} className="flex items-center gap-2.5 sm:gap-3 rounded-2xl border border-border bg-white/[0.02] p-2.5">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
